...
- [x] Add `Finalizer.set_stake_set()` / `get_stake_set()` — manual stake override (test compat; production path uses cache) — `finalizer/src/finalizer.rs`
- [x] Internal stake fetching in Finalizer — duplicate `StakeSnapshotCache` into finalizer crate (local → DataProvider, mirrors sentinel); `Finalizer::new` takes `data_provider` + `partition_id`; `get_stake_set_for_epoch()` resolves manual override → cache; `try_finalize_optimistic` fetches epoch stake for `BlockFinalized` gossip; `advance_epoch` invalidates cache — `finalizer/src/finalizer.rs`, `finalizer/src/stake_snapshot_cache.rs` — 8 new tests (5 cache + 3 finalizer)
...
**Remaining:** `Finalizer.initialize` — gossiper message handler subscription.
...
- ~~`Finalizer.try_finalize` — Standard (non-optimistic) path uses placeholder data: `total_stake=0, total_voters=0`, `previous_hash=vec![]` (see lines 246, 272). Optimistic path (`try_finalize_optimistic`) has same `previous_hash=vec![]` issue at line 363.~~ — **DONE** — both finalize paths now resolve `previous_hash` from the token's actual chain tip (`resolve_previous_hash` via `DataProvider::get_token`), and the standard path populates `total_stake`/`total_voters` from the epoch stake set (`resolve_stake_metrics`); genesis emits `vec![]` per the genesis convention
