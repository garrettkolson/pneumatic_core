...
- [ ] Core inconsistency: `Blockchain::validate_next_block` (src/blocks.rs:162) rejects **any** block when the chain is empty (`last_hash_in.len() == 0 → false`), so block 1 can never pass `Token::commit_block`; the committer gossip path bypasses it via raw `blockchain.add_block()` (committer/src/committer.rs:385). Decide on a genesis convention and reconcile
