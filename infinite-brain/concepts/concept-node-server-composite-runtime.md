...
staleness_signal: "Stale when node-server/src/node_server.rs, role_selector.rs, or role_dispatcher.rs change their layers or the plugin set"
...
- **NodeServer / epoch coordinator** (Phase 5, `node_server.rs:52-90`): owns the shared DI bundle (one build, shared across N plugins — generalizing the committer `main.rs` boot recipe), polls `EpochBoundaryDetector`, fans epoch advances and shutdown to the installed plugins; `roll_forward`/`recompute_role_set` let the role set change per epoch.

`build_runtime` (`node_server.rs:208-260`) tolerates boot failures (transport down, data service unreachable, stake-index warm-up) and boots anyway; only a missing environment is a hard error. `build_role_plugin` (`node_server.rs:421-545`) constructs each plugin from the shared bundle — `Archiver` has no plugin (`None`).
