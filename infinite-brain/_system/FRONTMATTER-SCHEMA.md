...
- **Type:** string
- **Required:** yes
- **Notes:** Represents the project, team, or domain area. Use kebab-case (e.g., `pneumatic`, `shielded`, `networking`). Enables scoped filtering and reduces cross-namespace collisions.
...
- **Format:** `MM/DD/YYYY`
- **Default:** `"Empty"`
- **Notes:** Date on which the node's content or conclusions were last reviewed or confirmed. Use `"Empty"` until first verification.

### verified_by
- **Type:** string
- **Default:** `"Empty"`
- **Notes:** Name or id of the person/agent who last verified this node. Use `"Empty"` until verified.

### staleness_signal
- **Type:** string
- **Required:** yes
- **Notes:** A specific, observable condition that, if met, indicates this node may be outdated, incorrect, or invalidated. Write it as a conditional statement (e.g., "If `src/shielded/` gains a new submodule, re-check this map"). Agents should monitor for staleness signals.
