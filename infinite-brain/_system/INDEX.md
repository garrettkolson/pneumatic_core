...
| `concept-epoch-and-staking` | Epochs, stake sets, and (stubbed) staking persistence | Epoch (epoch.rs:20) + StakeSet/ExecutorSet drive leader election and sharding; IStakingManager/IEpochReconciler exist but StubStakingManager is a no-op (epoch.rs:492). | 4 |
...
| `concept-leader-election` | Leader election: stake-weighted, domain-separated, tip-bound | LeaderSelector elects per epoch via stake-weighted selection (epoch.rs:314) over a seed SHA-256(domain‖epoch‖prev_hash‖extra); BlockProposer proposes from the pending pool. | 4 |
...
| `concept-shielded-pool-view` | ShieldedPoolView — read-only pool seam | Trait (pool_view.rs:44) exposing pool state read-only to validation; SimpleShieldedPoolView (line 58) is the stand-in until S5.4 swaps in the real Arc<ShieldedPool>. | 3 |
...
| `task-s5-4-real-pool-swap` | S5.4 — Composite node-server: real Arc<ShieldedPool> wiring | Replace the composite node's SimpleShieldedPoolView with the real shared Arc<ShieldedPool> in the node-server DI bundle, and add SignShielded/ShieldedVote to the finalizer role's action set. | 7 |
