...
| `concept-rns-transport` | RNS transport: the production inter-node wire (RnsNetwork over rns-net, Resource transfer for large payloads) | RNS is the production inter-node wire for all role-to-role traffic. Direct packet path caps at ~481 B, so real ~3.8 KB Messages must ride send_resource_to (RnsConnection does not auto-route — open fix tracked by the e2e pipeline task). | 4 |
