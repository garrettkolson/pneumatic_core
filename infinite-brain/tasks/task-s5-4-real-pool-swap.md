...
Implementation plan: `plans/S5.4-implementation-plan.md` (09/20/2026). Verified current state: the `FINALIZER_ACTIONS`/`handle` match arms already landed in S5.2 (node_server.rs:49–50, :664–697), so the remaining diff is the `build_runtime` DI swap (node_server.rs:310–312) + `build_role_plugin` arm threading + tests. Hard prerequisite: S5.3 (not landed as of 09/20/2026 — `committer/src/shielded_pool.rs` absent).
